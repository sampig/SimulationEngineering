...
public class CircusTrapeze {
    ...
    // initial
    double l = 5; // length of the string (meter).
    ...
    public CircusTrapeze() {
        start_time = 0.0;
        end_time = 10.0;
    }

    public CircusTrapeze(double start_time, double end_time) {
        super();
    ...
    public void start(int run_mode) {
        ...
        if (run_mode == STATUS_STATIC) {
            while (t <= end_time) {
                ...
                th_v += (-g / l * Math.sin(th)) * delta_time;
                t += delta_time;
